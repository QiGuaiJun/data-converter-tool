const modules = {
  ...
  api: { title: "API", icon: "API", color: "black", status: "未开放", body: "预留命令行/API 调用能力，便于桌面工具被外部脚本调度。" },
  docs: { title: "操作手册", icon: "DOC", color: "violet", body: "集中放置导入、导出、同步、连接、定时任务的操作说明。" },
  feedback: { title: "咨询建议反馈", icon: "?", color: "sky", status: "未开放", body: "记录使用问题、改进建议和待办需求。" },
};
...
function renderModulePage() {
  ...
  const ribbon = navItems
    .map(([id, href]) => {
      ...
      const badge = item.status ? `<span class="module-status">${item.status}</span>` : "";
      return `<a class="ribbon-item ${id === key ? "active" : ""}" href="${href}"><span class="ribbon-icon ${item.color}">${item.icon}</span><span>${item.title}</span>${badge}</a>`;
    })
    .join("");
  const tree = navItems
    .slice(0, 8)
    .map(([id, href]) => {
      const item = labels[id];
      const badge = item.status ? ` <span class="module-status">${item.status}</span>` : "";
      return `<a class="tree-node ${id === key ? "active" : ""}" href="${href}"><span>${item.icon}</span> ${item.title}${badge}</a>`;
    })
    .join("");
        ...
        <section class="app-workspace">
          <main class="module-placeholder">
            <header class="module-header">
              ...
              <div>
                <h1>${current.title}${current.status ? ` <span class="module-status">${current.status}</span>` : ""}</h1>
                <p>${current.body}</p>
              </div>
            </header>
            <section class="module-board">
              <div class="module-card">
                <strong>模块状态</strong>
                <p>${current.status
                  ? `此模块当前${current.status}，仅保留导航入口与页面骨架，请勿在正式流程中依赖本页提供的功能。`
                  : "此模块已拆分为独立页面，当前阶段保留入口与布局，功能开发时不会再挤占导入/导出页面。"}</p>
              </div>
              <div class="module-card">
                <strong>下一步</strong>
